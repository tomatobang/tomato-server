...
describe('test/service/user.test.js', () => {
  ...
  it('email username verify failed', async () => {
    const {
      emailRet,
      usernameRet,
    } = await ctx.service.user.emailUserNameVerify('333@qq.com', 'test');
  ...
  it('update user fields succeed', async () => {
    ...
    result = await ctx.service.user.updateSex(find_all[0]._id, 'man');
    ...
    result = await ctx.service.user.updateEmail(find_all[0]._id, 'new email');
